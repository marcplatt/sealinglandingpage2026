...
import {
  ...
  LandingSection,
  LandingTopBar
} from "../concrete-sealing-cowichan/LandingComponents";
...
const processTimeline = [
  "All sealing services are a two day process. One to wash the surface.",
  "The second to seal the surface.",
  "Sealing days require dry weather, making the schedule weather dependent.",
  "Pavers are the exception to the rule with a three day service time. After power washing joint sand is needed to refill the gaps between stones so that the hardscape does not shift in the future. Polymeric joint sand can be installed as well to further improve the longevity of the hardscape."
];
...
export async function KeywordLandingPage({
  ...
  const tracking: Partial<TrackingPayload> = {
    ...
    intent_service_type: config.intentServiceType
  };
    ...
    <main id="top" className="lp-shell">
      ...
      <LandingSection>
        <article className="lp-panel offer-panel">
          <h2>{config.cleaningTitle}</h2>
          <p>{config.cleaningBody}</p>
          <ul>
            {config.cleaningBullets.map((bullet) => (
      ...
      <LandingSection>
        <article className="lp-panel offer-panel">
          <h2>{config.sealingTitle}</h2>
          <p>{config.sealingBody}</p>
          <ul>
            {config.sealingBullets.map((bullet) => (
              <li key={bullet}>{bullet}</li>
            ))}
          </ul>
          <div className="offer-media-row" aria-label="Before and after media">
            <figure className="media-card">
              <Image
                ...
                alt="Concrete sealing before and after"
                width={619}
                height={362}
              />
      ...
      <LandingSection aria-label="FAQ">
        <LandingPanel className="faq-panel">
          ...
          <div className="faq-list">
            <details>
              <summary>How long after cleaning can you seal the surface?</summary>
              <p>Usually 24 to 48 hours depending on weather and moisture. We test before sealing.</p>
            </details>
            <details>
              <summary>How long does sealer last?</summary>
              <p>Most sealers protect for 2 to 5 years depending on product and traffic.</p>
            </details>
            <details>
              <summary>Will sealing make the surface slippery?</summary>
              <p>No. We can add anti-slip additives for sloped or high-traffic areas.</p>
            </details>
            <details>
              <summary>How soon can I use the surface after sealing?</summary>
              <p>Foot traffic is typically fine after 24 hours; vehicles after 48 to 72 hours.</p>
            </details>
            <details>
              <summary>Do you offer a package that includes both cleaning and sealing?</summary>
              <p>Yes. Sealing always includes our full cleaning prep as step one.</p>
            </details>
            <details>
              <summary>What areas do you serve?</summary>
              <p>Ladysmith, Cowichan Valley, and surrounding areas. We can also come out to Nanaimo and Victoria to seal.</p>
            </details>
          </div>
        </LandingPanel>
      </LandingSection>
